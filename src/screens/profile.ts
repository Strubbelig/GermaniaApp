...
function renderPersonal(m: Member): HTMLElement {
  const form = el('form', { class: 'card' }, [
    ...
    el('div', { class: 'grid2' }, [
      ...
      field('Verbindungsstatus', el('select', { name: 'corp_status' }, [
        opt('fux', 'Fux', m.corp_status),
        opt('bursch', 'Aktiver Bursch', m.corp_status),
        opt('philister', 'Philister', m.corp_status),
      ])),
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      await updateMyMember({
        ...
        corp_status: (f.get('corp_status') as Member['corp_status']) || 'bursch',
