...
insert into profession_category (name, slug, parent_id) values
    ('Finanzen','finance', null),
    ('Architektur','architecture', null)
on conflict (slug) do nothing;
insert into profession_category (name, slug, parent_id)
    select v.name, v.slug, p.id
    from (values
        ('Kinderkardiologie','pediatric-cardiology','medicine'),
        ('Dermatologie','dermatology','medicine'),
        ('Orthopädische Chirurgie','orthopedic-surgery','medicine'),
        ('Steuerrecht','tax-law','law'),
        ('Recht des geistigen Eigentums','ip-law','law'),
        ('Notariat','notary','law')
...
insert into address (member_id, label, is_primary, street, postal_code, city, region, country_code, geo) values
 ...
 ('a0000000-0000-0000-0000-000000000002','home',true,'Leopoldstrasse 12','80802','München','Bayern','DE',  st_setsrid(st_makepoint(11.5820,48.1351),4326)::geography),
 ...
 ('a0000000-0000-0000-0000-000000000004','home',true,'Bahnhofstrasse 3','8001','Zürich','Zürich','CH',     st_setsrid(st_makepoint(8.5417,47.3769),4326)::geography),
 ('a0000000-0000-0000-0000-000000000005','home',true,'Ringstrasse 22','1010','Wien','Wien','AT',       st_setsrid(st_makepoint(16.3738,48.2082),4326)::geography),
 ...
 ('a0000000-0000-0000-0000-000000000009','home',true,'Domkloster 4','50667','Köln','NRW','DE',          st_setsrid(st_makepoint(6.9603,50.9375),4326)::geography),
...
insert into relative (member_id, relationship, first_name, last_name, gender, date_of_birth, street, postal_code, city, country_code, geo) values
 ...
 ('a0000000-0000-0000-0000-000000000004','child','Jonas','Vogel','male','2010-03-17','Bahnhofstrasse 3','8001','Zürich','CH', st_setsrid(st_makepoint(8.5417,47.3769),4326)::geography),
 ('a0000000-0000-0000-0000-000000000004','child','Mia','Vogel','female','2014-07-02','Bahnhofstrasse 3','8001','Zürich','CH', st_setsrid(st_makepoint(8.5417,47.3769),4326)::geography),
 ...
 ('a0000000-0000-0000-0000-000000000009','child','Paul','Hoffmann','male','2008-01-05','Domkloster 4','50667','Köln','DE', st_setsrid(st_makepoint(6.9603,50.9375),4326)::geography);
